...
seed:
	...
	@echo "→ cassettes de scénario E2E, bancs (U29a2/U29a4/U29b2/U29c2, générées : make seed-e2e)"
	@for f in e2e_banc_entrepot.jsonl e2e_banc_depot.jsonl e2e_banc_ctf.jsonl e2e_banc_tau.jsonl; do \
	  if [ -s tests/fixtures/llm/cassettes/$$f ]; then \
	    echo "  $$f : $$(wc -l < tests/fixtures/llm/cassettes/$$f) échanges"; \
	  else \
	    echo "  $$f ABSENTE — lancez « make seed-e2e » puis relancez la pile"; \
