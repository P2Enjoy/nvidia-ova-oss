...
- **Modèle de travail : `qwen3.6:35b`** (défaut H3.1 ; capacités `completion`, `tools`, `thinking`, `vision`). Modèles observés sur l'endpoint au 2026-08-30 : `qwen3.6:35b`, `qwen3.8:27b` (apparu après le 2026-08-27, non évalué — changer `AVO_MODEL` seulement sur décision du responsable) et `all-minilm:latest` (embeddings uniquement).
...
- **Budget d'inférence (règle du responsable, 2026-09-01)** : illimité TANT QUE le
  modèle de travail est `qwen3.6:35b` ET que l'inférence passe par le gateway LLM du
  responsable (l'endpoint fourni, directement ou via le pont 443). Changer de modèle
  ou d'infrastructure d'inférence rouvre l'arbitrage de dépense (cas 2 de « Demande
