...
| `avo.llm` | H4 | client d'inférence (Ollama natif `/api/chat`), erreurs typées, retries bornés, transport injectable ; limiteur de concurrence par endpoint à jetons de fichiers (`concurrence.py`, §H4.9 — mode live uniquement, 429 patienté) |
...
| `avo.bancs` | S1–S19 | bancs d'affinage du harnais : environnements de mesure déterministes branchés comme adaptateurs minces sur le contrat `Environnement` — banc a (`skillexec/` : Entrepôt et Dépôt logiciel, générateurs seedés, score continu, conditions bruit et dérive §S5.5), banc b (`ctf/` : cinq familles de défis seedées, terminal bash confiné à exécuteurs `conteneur`/`processus`, pass@1, schéma de Σ `ctf`) et banc c (`tau/` : base Détail SQLite seedée et outillée, scénario à intention et éligibilité, évaluateur d'état final, utilisateur simulé scripté/LLM choisi par le mode) ; adaptateurs de boucle livrés (outils `action`, contexte de tâche en message système, relevé `banc.json`) et sous-commande CLI `banc` (`--bruit`, `--derive` pour le banc a ; `--executeur` pour le banc b, `conteneur` requis en live) — le dispatch et les mots des bancs vivent sous `avo.bancs`, la CLI du noyau reste générique |
