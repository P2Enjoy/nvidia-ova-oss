...
- Fait : **U25 (campagne ARC officielle) livrée le 2026-09-05** — les 25 jeux de l'ensemble public joués en live sous les plafonds du responsable, un scorecard fermé par jeu, rapport final agrégé `docs/rapports/u25-t1-final.md`. Score global : RHAE 0,00, le plafond de temps (1 200 s/jeu) ayant lié sur tous les jeux — la lecture du score est bornée par ce budget. Toute nouvelle campagne relève du cycle permanent U31 (affinage sur les bancs U29, déclencheur consigné).
...
| `AVO_SUP_SONDE_FRAICHE` | Sonde fraîche (H10.4) : à l'intervention, un appel séparé SANS historique (énoncé de tâche + dernière observation) produit une proposition indépendante, jointe au message `[SUPERVISEUR]` comme alternative non ancrée | booléen | non (défaut `true`) | `true` |
...
| `AVO_FLUX_RECUPERATION` | Récupération du flux coupé (H4.10) : à l'épuisement des retries sur coupures mi-flux, le meilleur partiel reçu est rendu comme réponse tronquée au lieu de tuer l'épisode | booléen | non (défaut `true`) | `true` |
| `AVO_FLUX_RECUP_MIN_CARACTERES` | Caractères minimaux (`content` + `reasoning`) du partiel récupérable (H4.10) ; en dessous, l'erreur de transport se propage | entier ≥ 0 | non (défaut `200`) | `200` |
